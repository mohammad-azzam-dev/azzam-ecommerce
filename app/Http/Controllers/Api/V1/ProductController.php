...
class ProductController extends Controller
{
    ...
    public function get_searched_products(Request $request)
    {
        ...
        if (count($products['products']) == 0) {
            ...
            $ids = App\Model\Translation::where(['key' => 'name'])->where(function ($query) use ($key) {
    ...
    {
        try {
            $products = Helpers::product_data_formatting(Product::orderBy('id', 'desc')->active()->withCount(['wishlist'])->with(['rating'])->where('discount', '>', 0)->get(), true);
            return response()->json($products, 200);
        } catch (\Exception $e) {
